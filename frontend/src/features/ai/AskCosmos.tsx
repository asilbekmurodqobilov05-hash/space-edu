  ...
  return (
    ...
    >
      ...
      <path
        ...
        strokeLinecap="round"
      />
      {/* Central quatrefoil / güll motif (simplified) */}
...
function buildSystemInstruction(context: string | undefined, mode: ChatMode): string {
  const modeBlock =
    mode === "quiz"
...
function messagesToContents(msgs: ChatMessage[]) {
  return msgs.map((m) => ({
...
export default function AskCosmos() {
  ...
  useEffect(() => {
    if (context && messages.length === 1) {
      setMessages([
        {
          role: "ai",
          text: `You're on “${context}”. I'm your Space edu tutor — ask a question, or tap a quick prompt below.`,
        },
      ]);
    }
  }, [context]);
  ...
  const handleSend = async () => {
    ...
    const newMessages: ChatMessage[] = [...trimmedHistory, { role: "user" as const, text: userMessage }];
        ...
        {isOpen && (
          ...
          >
            ...
            <div className="px-3 pt-2 flex flex-wrap gap-1.5 border-b border-white/5 bg-space-900/40">
              {(
                [
                  ...
                  { id: "deep" as const, label: "Deep dive", Icon: Microscope },
                ] as const
              ).map(({ id, label, Icon }) => (
                <button
                  key={id}
            ...
            <div className="p-3 border-t border-white/10 bg-space-800/90 space-y-2">
              ...
              <p className="text-[10px] text-white/35">Enter to send · Shift+Enter for newline · Multi-turn memory (last {MAX_HISTORY_TURNS} messages)</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
