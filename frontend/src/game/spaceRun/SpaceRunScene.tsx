...
import { useSpaceArcadeStore, type ShipSkinId } from "./spaceArcadeStore";
import { playCoinSound, playExplosionSound } from "./spaceRunSounds";

type ObType = "meteor" | "coin" | "power";
type PowerKind = "shield" | "slow" | "magnet";
...
const SKIN_COLORS: Record<ShipSkinId, string> = {
  default: "#38bdf8",
  ruby: "#fb7185",
  lime: "#a3e635",
  solar: "#fbbf24",
...
function shallowCopyObs(src: Obstacle[]) {
  return src.map((o) => ({ ...o }));
}
...
function CameraRig({
  ...
  shipX: MutableRefObject<number>;
  shipY: MutableRefObject<number>;
...
export function SpaceRunScene({
  inputRef,
  runningRef,
}: {
  inputRef: MutableRefObject<{ left: boolean; right: boolean; up: boolean; down: boolean; boost: boolean }>;
  ...
  const [renderObs, setRenderObs] = useState<Obstacle[]>([]);
  ...
  useFrame((_, delta) => {
    ...
    if (spawnC.current >= coinEvery) {
      ...
      if (roll < 0.06 && magnetT.current <= 0 && slowT.current <= 0) {
        const kinds: PowerKind[] = ["shield", "slow", "magnet"];
