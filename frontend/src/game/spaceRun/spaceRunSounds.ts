...
export function playExplosionSound() {
  ...
  for (let i = 0; i < buffer.length; i++) data[i] = (Math.random() * 2 - 1) * Math.exp(-i / (buffer.length * 0.25));
...
let musicOscs: { stop: () => void }[] = [];
let engineNodes: { stop: () => void }[] = [];

/** Simple ambient arpeggio — low CPU */
...
/** Constant low engine hum for forward thrust feel */
