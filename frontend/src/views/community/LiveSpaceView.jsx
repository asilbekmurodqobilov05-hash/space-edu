        ...
        <div className="flex justify-between items-end flex-1 my-4 gap-4">
          <div className="w-full max-w-sm pointer-events-auto flex flex-col gap-4">
            <div className="rounded-xl border border-white/10 bg-black/45 px-3 py-2 text-[10px] uppercase tracking-widest text-cyan-300 backdrop-blur-md">
              {isRealData ? t('liveSpace', 'liveTle') : t('liveSpace', 'fallbackTle')}
            </div>
                  ...
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      <div className="text-[10px] text-neon-blue font-bold uppercase tracking-widest mb-1 flex items-center gap-1.5">
                        <Crosshair className="w-3 h-3" /> {t('liveSpace', 'targetLocked')}
                      </div>
                  ...
                  <div className="space-y-2">
                    <div className="bg-black/50 p-3 rounded-xl border border-white/5 flex justify-between items-center">
                      <div className="flex items-center gap-2 text-gray-400 text-[11px] font-bold uppercase tracking-widest">
                        <Activity className="w-3.5 h-3.5 text-neon-purple" /> {t('liveSpace', 'altitude')}
                      </div>
                    ...
                    <div className="bg-black/50 p-3 rounded-xl border border-white/5 flex justify-between items-center">
                      <div className="flex items-center gap-2 text-gray-400 text-[11px] font-bold uppercase tracking-widest">
                        <Gauge className="w-3.5 h-3.5 text-orange-400" /> {t('liveSpace', 'velocity')}
                      </div>
                      <div className="font-mono text-sm text-white">
                    ...
                    <div className="grid grid-cols-2 gap-2 mt-2">
                      <div className="bg-black/50 p-2.5 rounded-xl border border-white/5 flex flex-col">
                        <span className="text-gray-500 text-[9px] font-bold uppercase tracking-widest mb-1">{t('liveSpace', 'inclination')}</span>
                        <span className="font-mono text-sm text-white">{metrics.inc} deg</span>
                      </div>
                      <div className="bg-black/50 p-2.5 rounded-xl border border-white/5 flex flex-col">
                        <span className="text-gray-500 text-[9px] font-bold uppercase tracking-widest mb-1">{t('liveSpace', 'latLon')}</span>
          ...
          <div className="hidden lg:block w-72 pointer-events-auto">
            <div className="backdrop-blur-xl bg-black/55 border border-white/10 rounded-2xl p-4 shadow-2xl">
              <div className="text-[10px] uppercase tracking-widest text-cyan-300 font-bold mb-3">{t('liveSpace', 'satDetails')}</div>
              {selectedSat ? (
                <div className="space-y-3 text-xs">
                  <div className="rounded-xl border border-white/10 bg-black/40 p-3">
                    <div className="text-[10px] text-gray-400 uppercase tracking-widest mb-2">{t('liveSpace', 'identity')}</div>
                    <div className="grid grid-cols-2 gap-x-2 gap-y-1 text-gray-300">
                      <span className="text-gray-500">{t('liveSpace', 'name')}</span><span className="text-right truncate">{selectedSat.name}</span>
                      <span className="text-gray-500">NORAD</span><span className="text-right">{selectedSat.id}</span>
                      <span className="text-gray-500">{t('liveSpace', 'type')}</span><span className="text-right">{selectedSat.type}</span>
                      <span className="text-gray-500">{t('liveSpace', 'object')}</span><span className="text-right">{satCatalog?.OBJECT_TYPE || t('common', 'unknown')}</span>
                      <span className="text-gray-500">{t('liveSpace', 'country')}</span><span className="text-right">{satCatalog?.COUNTRY || t('common', 'unknown')}</span>
                    </div>
                  </div>

                  <div className="rounded-xl border border-white/10 bg-black/40 p-3">
                    <div className="text-[10px] text-gray-400 uppercase tracking-widest mb-2">{t('liveSpace', 'orbitalTelemetry')}</div>
                    <div className="grid grid-cols-2 gap-x-2 gap-y-1 text-gray-300">
                      <span className="text-gray-500">{t('liveSpace', 'altitude')}</span><span className="text-right">{metrics?.alt || '--'} km</span>
                      <span className="text-gray-500">{t('liveSpace', 'velocity')}</span><span className="text-right">{metrics?.vel || '--'} km/h</span>
                      <span className="text-gray-500">{t('liveSpace', 'inclination')}</span><span className="text-right">{metrics?.inc || '--'} deg</span>
                      <span className="text-gray-500">{t('liveSpace', 'latitude')}</span><span className="text-right">{metrics?.lat || '--'}</span>
                      <span className="text-gray-500">{t('liveSpace', 'longitude')}</span><span className="text-right">{metrics?.lon || '--'}</span>
                      <span className="text-gray-500">{t('liveSpace', 'eccentricity')}</span><span className="text-right">{selectedSat.satrec?.ecco?.toFixed?.(6) || '--'}</span>
                      <span className="text-gray-500">{t('liveSpace', 'period')}</span><span className="text-right">{selectedSat.satrec?.no ? (1440 / (selectedSat.satrec.no * 1440 / (2 * Math.PI))).toFixed(1) : '--'} min</span>
                    </div>
                  </div>

                  <div className="rounded-xl border border-white/10 bg-black/40 p-3">
                    <div className="text-[10px] text-gray-400 uppercase tracking-widest mb-2">{t('liveSpace', 'launchMission')}</div>
                    <div className="grid grid-cols-2 gap-x-2 gap-y-1 text-gray-300">
                      <span className="text-gray-500">{t('liveSpace', 'intlDesig')}</span><span className="text-right">{satCatalog?.INTLDES || t('common', 'unknown')}</span>
                      <span className="text-gray-500">{t('liveSpace', 'launchDate')}</span><span className="text-right">{satCatalog?.LAUNCH || t('common', 'unknown')}</span>
                      <span className="text-gray-500">{t('liveSpace', 'launchSite')}</span><span className="text-right">{satCatalog?.SITE || t('common', 'unknown')}</span>
                      <span className="text-gray-500">{t('liveSpace', 'launchPiece')}</span><span className="text-right">{satCatalog?.LAUNCH_PIECE || t('common', 'unknown')}</span>
                      <span className="text-gray-500">{t('liveSpace', 'status')}</span><span className="text-right">{satCatalog?.OPS_STATUS || t('common', 'unknown')}</span>
                      <span className="text-gray-500">{t('liveSpace', 'crewOnboard')}</span><span className="text-right">{selectedSat.type === 'ISS' ? t('liveSpace', 'crewed') : t('liveSpace', 'uncrewed')}</span>
                      <span className="text-gray-500">{t('liveSpace', 'launchVehicle')}</span><span className="text-right">{satCatalog?.LAUNCH_VEHICLE || t('liveSpace', 'notPublished')}</span>
                    </div>
                  </div>
                  {catalogLoading && <div className="text-[11px] text-gray-500">{t('liveSpace', 'loadingCatalog')}</div>}
                </div>
              ) : (
                <div className="text-xs text-gray-400">{t('liveSpace', 'selectSat')}</div>
              )}
            </div>
          </div>
        </div>
      ...
      <section className="relative z-20 px-4 sm:px-6 py-8 space-y-6 bg-gradient-to-b from-[#020205] to-black">
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          <div className="xl:col-span-2 rounded-2xl border border-white/10 bg-black/40 backdrop-blur-xl p-4">
            <h3 className="text-sm font-bold tracking-wider uppercase text-cyan-300 mb-3">{t('liveSpace', 'nasaLive')}</h3>
          ...
          <div className="rounded-2xl border border-white/10 bg-black/40 backdrop-blur-xl p-4">
            <h3 className="text-sm font-bold tracking-wider uppercase text-cyan-300 mb-3">{t('liveSpace', 'nasaApod')}</h3>
            <NasaApod />
          </div>
        </div>
        <div className="rounded-2xl border border-white/10 bg-black/40 backdrop-blur-xl p-4">
          <h3 className="text-sm font-bold tracking-wider uppercase text-cyan-300 mb-3">{t('liveSpace', 'upcomingMissions')}</h3>
          <UpcomingLaunches />
        </div>
      </section>
    </div>
