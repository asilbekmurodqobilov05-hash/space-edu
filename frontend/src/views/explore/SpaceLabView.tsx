...
const RealisticEarth = ({ radius = 5, position = [0, 0, 0] }: { radius?: number, position?: [number, number, number] }) => {
...
const RocketEngineeringLab = () => {
  ...
  const [payloadType, setPayloadType] = useState<'fairing' | 'capsule'>('fairing');
  const [stage1Type, setStage1Type] = useState<'standard' | 'heavy'>('standard');
...
const LaunchAnimator = ({ launchState, rocketRef, stage1Ref, speed }: { launchState: string, rocketRef: React.RefObject<THREE.Group>, stage1Ref: React.RefObject<THREE.Mesh>, speed: number }) => {
  useFrame((state) => {
...
const RocketLaunchSimulator = () => {
  ...
  const stage1Ref = useRef<THREE.Mesh>(null);
...
const MeteorShower = ({ intensity }: { intensity: number }) => {
...
const PlanetaryProcessesLab = () => {
  const { t } = useTranslation();
  const [activeEvent, setActiveEvent] = useState<'none' | 'meteor' | 'volcano' | 'dust'>('none');
...
const ParticleSystem = ({ count, color, size, radius }: { count: number, color: string, size: number, radius: number }) => {
...
const UniverseChangesSimulator = () => {
  const { t } = useTranslation();
  const [stage, setStage] = useState<'nebula' | 'star' | 'supernova' | 'blackhole'>('nebula');
...
const SatelliteControlSimulator = () => {
  ...
  const [satelliteType, setSatelliteType] = useState<'iss' | 'tiangong' | 'dragon' | 'soyuz'>('iss');
