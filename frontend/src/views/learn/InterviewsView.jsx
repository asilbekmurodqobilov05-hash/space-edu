      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'relative', zIndex: 1 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          <span style={{ fontSize: '13px', color: 'rgba(255,255,255,0.4)' }}>
            {t('learnViews', 'interviewCount').split('{count}')[0]}
            <span style={{ color, fontWeight: 700 }}>{topic.sections ? topic.sections.reduce((acc, s) => acc + s.lessons.length, 0) : topic.lessons.length}</span>
            {t('learnViews', 'interviewCount').split('{count}')[1]}
          </span>
          <div
            style={{
...
export default function InterviewsView() {
  ...
  return (
    <div className="pt-24 pb-20" style={{ minHeight: '100vh', background: 'transparent' }}>
      <SectionPageHeader title={t('learnViews', 'interviewsTitle')} color={color} />
