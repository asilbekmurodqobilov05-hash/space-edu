...
export interface LessonSection {
  id: string;
  type: 'explanation' | 'visual' | 'practice' | 'quiz' | 'video' | 'worked-example' | 'simulation';
  title: string;
  content?: string[];
  imageUrl?: string;
  imageAlt?: string;
...
export const learningData: Level[] = [
  {
    id: 'level-1',
    levelNumber: 1,
...
export const getLessonById = (lessonId: string): { lesson: Lesson, unitId: string, levelId: string } | undefined => {
  for (const level of learningData) {
...
export const getNextLesson = (unitId: string, currentLessonId: string): Lesson | undefined => {
